...
int main (int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if ( option == 1 )
        {
            ...
            // STUDENT WORK
	        char command_input[] = "ifconfig";
	        char command_output[COMMAND_BUFF];
	        char line[COMMAND_BUFF];

	        if (system_info){
		        while (fgets (line, sizeof(line), system_info)){
			    strncpy(command_output + strlen(command_output), line, strlen(line));
		        }
	        }
            // Writing proper header
	        strncpy(message_buff, "HTTP/1.1 200 Ok\n", strlen("HTTP/1.1 200 Ok\n"));
	        strncpy(message_buff + strlen(message_buff), "Server: Chloe's COEN146 Web Server\n", strlen("Server: Chloe's COEN146 Web Server\n"));
	        ...
	        if ((net_bytes_read = write(n_socket_fd, message_buff, strlen(message_buff))) < 0){
                printf ("Error writing header.\n");
                return 0;
            }

	        if((net_bytes_read = write(n_socket_fd, command_output, strlen(command_output))) < 0){
        ...
        else if (option == 2)
        {
            ...
            if (stat(HTML_FILE, &sbuf) < 0) {
                // STUDENT WORK
                shutdown(n_socket_fd, SHUT_RDWR);
		        close(n_socket_fd);
            }
            ...
            else
            {
		        ...
		        while (packet_size = fread(file_buff,1,FILE_READ, source_file), packet_size > 0){
			        strncpy(line+strlen(line),file_buff,packet_size);
		        }
		        strncpy(message_buff, "HTTP/1.1 200 Ok\n", strlen("HTTP/1.1 200 Ok\n"));
		        ...
		        if ((net_bytes_read = write(n_socket_fd, line, strlen(line))) < 0){
                    printf("Error writing body.\n");
                    return 1;
                }     
