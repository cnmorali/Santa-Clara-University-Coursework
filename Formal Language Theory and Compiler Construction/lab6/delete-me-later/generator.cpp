...
static std::map<double, Label> reals;
...
# define isFP(expr) (expr->type().specifier() == DOUBLE)
...
void generateGlobals(Scope *scope)
{
    ...
    for (auto rl: reals) {
            cout << "\t" << rl.second << ":\t.double\t" << rl.first << endl;
    }
}
...
void Real::operand(ostream &ostr) const {
    ...
    double reald = stod(_value);
    if (reals.find(reald) == reals.end())
        reals.insert({reald, lab});
