...
void Integer::write(ostream &ostr) const
{
    ostr << _value;
}

void Real::write(ostream &ostr) const
{
    ostr << _value;
}
...
void For::write(ostream &ostr) const
{
    ostr << "(for " << _init << " " << _expr;
    ostr << " " << _stmt << ")";
}
...
{
    unsigned num = _id->type().parameters()->types.size();
