...
Expression::Expression(const Type &type)
    : _type(type), _lvalue(false), offset(0), hasCall(false), reg(nullptr)
{
}
...
{
    hasCall = _left->hasCall | _right->hasCall;
}
...
/*
 * Function:	Expression:isReal (accessor)
 *
 * Description:	Return false since most expressions are not reals.
 */

bool Expression::isReal(double &value) const
{
    return false;
}
...
/*
 * Function:	Expression:isInteger (accessor)
 *
 * Description:	Return false since most expressions are not integers.
 */

bool Expression::isInteger(unsigned &value) const
{
    return false;
}
...
/*
 * Function:	Real::isReal (accessor)
 *
 * Description:	Return true since this is in fact a real.
 */

bool Real::isReal(double &value) const
{
    value = strtod(_value.c_str(), NULL);
    return true;
}


/*
 * Function:	Integer::isInteger (accessor)
 *
 * Description:	Return true since this is in fact an integer.
 */

bool Integer::isInteger(unsigned &value) const
{
