...
/*
 * Function:	optimize_cast (private)
 *
 * Description:	Perform some optimizations on a type cast so as to reduce
 *		the number of cases the code generator must consider.
 *
 *		Most floating-point instructions do not accept immediates,
 *		so integer literals are replaced with real literals, and
 *		vice versa.  The latter is not always safe since the target
 *		machine may perform a different conversion (e.g.,
 *		truncation or rounding) than we do here.
 *
 *		Also, most floating-point instructions that do accept
 *		integer values, either in memory or in registers, do not
 *		support 8-bit values, only 32-bit values, so we interpose
 *		an explicit cast to or from an integer if necessary.
 */

static void optimize_cast(Expression *&expr, const Type &type)
{
    double dval;
    unsigned ival;


    /* (double) int-literal becomes real-literal */

    if (expr->isInteger(ival) && type == real)
	expr = new Real(ival);


    /* (double) char-expression becomes (double) (int) char-expression */

    else if (expr->type() == character && type == real)
	expr = new Cast(expr, integer);


    /* (int) real-literal or (char) real-literal becomes int-literal */

    else if (expr->isReal(dval) && (type == integer || type == character))
	expr = new Integer(dval);


    /* (char) double-expression becomes (char) (int) double-expression */

    else if (expr->type() == real && type == character)
	expr = new Cast(expr, integer);
}
 ...
 *		done to determine the validity of the cast.
 */

static Expression *&cast(Expression *&expr, const Type &type)
{
    optimize_cast(expr, type);
...
/*
 * Function:	upcast (private)
 *
 * Description:	Convert the left operand to the type of the right operand
 *		but only by upcasting.  At the minimum, perform arithmetic
 *		promotion on the left operand.
 */

static Expression *&upcast(Expression *&left, Expression *&right)
{
    const Type &t1 = left->type();
    const Type &t2 = right->type();


    if ((t1 == character || t1 == integer) && t2 == real)
	return cast(left, real);
...
static Expression *scale(Expression *expr, unsigned size)
{
    ...
    if (expr->isInteger(value)) {
	delete expr;
	return new Integer(value * size);
    }

    return new Multiply(expr, new Integer(size), integer);
}
 ...
 *		have type int, and the result has type T.
 */
 ...
 *		desired type must both be numeric types or both be pointer
 *		types.
 */
 ...
 *		type, and the result type double if either operand has type
 *		double and has type int otherwise.
 */

static Type
checkMultiplicative(Expression *&left, Expression *&right, const string &op)
{
    const Type &t1 = decay(upcast(left, right));
    const Type &t2 = decay(upcast(right, left));
    Type result = error;
 ...
 *
 * Description:	Check a remainder expression: LEFT % RIGHT.  The operands
 *		undergo the usual conversions and then must both have type
 ...
 *		operand has type double and has type int otherwise.  If one
 *		operand has a pointer type and the other has type int, then
 *		the result has that pointer type.
 */
 ...
 *		first undergo the usual conversions.  If both then have
 *		numeric types, then the result has type double if either
 *		operand has type double and has type int otherwise.  If the
 *		left operand has a pointer type and the right operand has
 *		type int, then the result has that pointer type.  If the
 *		left and right operands have identical pointer types, then
...
Expression *checkSubtract(Expression *left, Expression *right)
{
	...
	expr = new Divide(expr, new Integer(t1.dereference().size()), integer);
