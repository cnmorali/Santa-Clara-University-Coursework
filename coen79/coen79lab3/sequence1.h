...
namespace coen79_lab3 {
	...
	class sequence {
			...
			// Member functions
			void start(); // Go to front of sequence
			void end(); // Go to end of sequence (current item becomes last item)
			void last();
			void advance();
			void retreat();
			void insert(const value_type& entry);
			void attach(const value_type& entry);
			void remove_current();
			void insert_front(const value_type& entry); // To insert a new value at the front of the sequence
			void attach_back(const value_type& entry); // To insert a value at the back of the sequence
			void remove_front(); // To remove the value at the front of the sequence
			void  operator +=(const sequence& rhs);
