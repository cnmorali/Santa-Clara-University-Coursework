...
namespace coen79_lab8
{
	...
	block_pointers = new value_type*[bp_array_size];
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
			...
			if(source.front_ptr == &(source.block_pointers[bp_array_index])[block_item_index]){ // Check if inserted item is item pointed to by front pointer
				front_ptr = &(block_pointers[bp_array_index])[block_item_index]; // Setting data of current datablock index equal to the data at the same index in deque
			}
			if(source.back_ptr == &(source.block_pointers[bp_array_index])[block_item_index]){ // Check if inserted item is item pointed to by back pointer
				back_ptr = &(block_pointers[bp_array_index])[block_item_index]; // Setting data of current datablock index equal to the data at the same index in deque
			}  
              }
            }
        }
    ...
    deque<Item>::~deque () {
        ...
        first_bp = last_bp = block_pointers_end = block_pointers = NULL; // Setting all pointers equal to NULL
        front_ptr = back_ptr = NULL;
    }
    ...
    void deque<Item>::clear () {
        ...
        // Clear the data blocks
        // STUDENT WORK...
        for(size_type i = 0; i < bp_array_size; ++i){ // Setting all entries equal to NULL
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // STUDENT WORK...
         	*first_bp = new value_type[block_size]; // Creating new block of block_size with new entry as first entry
		front_ptr = *first_bp;
		back_ptr = *last_bp;
		*back_ptr = entry;
        }
        ...
        {
            // STUDENT WORK...
        	--front_ptr; // Decrementing pointer
		*front_ptr = entry; // Assigning value
        }
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            // STUDENT WORK...
		*first_bp = new value_type[block_size]; // Create new block with block_size with new entry as first entry
		front_ptr = *last_bp;
		back_ptr = *last_bp;
		*back_ptr = entry;
        }
        ...
        {
            // STUDENT WORK...
		++back_ptr; // Incrementing pointer
		*back_ptr = entry; // Assigning value
        }
        ...
        else if ((back_ptr == ((*last_bp) + (block_size - 1))) && (last_bp != block_pointers_end))
        {
            // STUDENT WORK...
		++last_bp;
		*last_bp = new value_type[block_size]; // Creating new block with new entry as back entry
		back_ptr = *last_bp; 
		*back_ptr = entry;
        }
        ...
        {
            // STUDENT WORK...
		reserve(); // Reserve space for more block pointers create new block with new entry as back entry
		++last_bp;
    ...
    void deque<Item>::pop_front()
    {
        ...
        {
            // STUDENT WORK...
		delete []*first_bp; // Delete data block
		*first_bp = NULL; // Increment first_bp
		++first_bp;
		front_ptr=*first_bp; // Set front_ptr to start of data block
        }
    ...
    void deque<Item>::pop_back()
    {
        ...
        {
            // STUDENT WORK...
		delete[] back_ptr; // delete back
		*last_bp = NULL;
		--last_bp;
		back_ptr = *last_bp +(block_size-1); // Moving back_ptr to new spot
        }
        else
        {
            --back_ptr;
